...
def launch_slurm_ls5(serialcmd='', script_name='', runtime='01:00:00',
    ...
    if max_cores_per_node is None:
        ...
        if use_hyperthreading is False:
            print(
                "Enabling hyperthreading (--ht) would double the available cores per node.")
        sys.exit()
    ...
    if len(serialcmd) > 0:
        ...
        if not nodes:
            ...
            print(
                'Number of compute nodes not specified - estimating as %d' % nodes)
    ...
    qsubfile.write(
        '#SBATCH -o {0}.o%j # Name of the output file (eg. myMPI.oJobID)\n'.format(jobname))
    ...
    if not test:
        process = subprocess.Popen(
            'sbatch %s' % qsubfilepath, shell=True, stdout=subprocess.PIPE)
